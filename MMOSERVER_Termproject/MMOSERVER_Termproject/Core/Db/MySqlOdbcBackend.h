...
// MySQL(ODBC) DB 백엔드. IDbBackend 구현.
...
class MySqlOdbcBackend : public IDbBackend {
public:
    explicit MySqlOdbcBackend(const std::string& conn_str);
    ~MySqlOdbcBackend() override;

    MySqlOdbcBackend(const MySqlOdbcBackend&) = delete;
    MySqlOdbcBackend& operator=(const MySqlOdbcBackend&) = delete;
